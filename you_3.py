...
def compute_adaptive_damping(jac):
    ...
    if s[-1] == 0:  # Check for singularity
        print("Warning: Jacobian is singular. Using maximum damping.")
        return 50.0  # Return a high damping factor to stabilize
...
def validate_ik_solution(joints, target_pose, DH_params):
    ...
    current_pose = np.array(trans_EF_eval(joints, DH_params)).astype(np.float64)
...
def i_kine(joints_init, target, DH_params, max_iters=1000, error_threshold=1e-3, no_rotation=False, joint_lims=True):
